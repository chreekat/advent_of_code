...
data Gridmark = Start | Gridmark (NEMap Int Int)
    deriving (Eq, Show)

singleMark i k = Gridmark (NM.singleton i k)

instance Semigroup Gridmark where
    Start <> _ = Start
    _ <> Start = Start
    Gridmark s <> Gridmark s' = Gridmark (NM.unionWith min s s')

instance Monoid Gridmark where
    mempty = Start

hasCrossing Start = False
hasCrossing (Gridmark s) = NM.size s > 1

type Grid = Map Loc Gridmark

emptyGrid = M.fromList [(Loc 0 0, Start)]

data Loc = Loc { xLoc :: Int, yLoc :: Int }
    deriving (Eq, Show, Ord)

addX, addY :: Int -> Loc -> Loc
addX i (Loc x y) = Loc (x+i) y
addY i (Loc x y) = Loc x (y+i)

data LocGrid = LocGrid
    { locGridLoc :: Loc
    , locGridGrid ::  Grid
    , locGridDist :: Int
    }
...
emptyLocGrid = LocGrid (Loc 0 0) emptyGrid 0
...
data Seg = Seg Dir Int
    deriving (Eq, Show)

type WireId = Int

addSegment :: WireId -> Seg -> LocGrid -> LocGrid
addSegment wid (Seg dir len) (LocGrid loc grid dist) =
    let op = dirOp 1 dir
        locs = tail $ take (len + 1) (iterate op loc)
        dists = [dist + 1 ..]
        newMarks = M.fromList (zip locs (map (singleMark wid) dists))
    in LocGrid
        (dirOp len dir loc)
        (M.unionWith (<>) grid newMarks)
        (dist + len)

addWire :: WireId -> [Seg] -> LocGrid
addWire wid = foldl' (flip (addSegment wid)) emptyLocGrid

buildGrid :: [[Seg]] -> Grid
buildGrid
    = M.unionsWith (<>) . map locGridGrid . zipWith addWire [0..]

manhattanDist (Loc x y) = abs x + abs y

closestCrossingDist :: Grid -> Int
closestCrossingDist
    = minimum . map manhattanDist . M.keys . M.filter hasCrossing

crossingDelay :: Gridmark -> Int
crossingDelay Start = error "wat"
crossingDelay (Gridmark m) = NM.foldr1 (+) m

minimumCrossingDelay :: Grid -> Int
minimumCrossingDelay
    = minimum . map crossingDelay . M.elems . M.filter hasCrossing

readSegments :: String -> [Seg]
readSegments = map oneSeg . splitOn1 ','
    where oneSeg (c:rest) = Seg (read [c]) (read rest)
